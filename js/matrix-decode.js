      ...
      <div class="matrix-wrapper">
        ...
        ${[ ...Array(m).keys() ].map(i => html`<div class="m m-${i}">
          ${[ ...Array(n).keys() ].map(j => html`<div class="n n-${j}" @click=${event => this.toggleValueAt(i, j)}>
            <input @click=${event => this.lockBg && event.target.select()} @change=${event => this.inputValidation(event, i, j)} ?readonly=${!this.lockBg} class="values ${this.bg(i, j) > 0 ? "invert" : "normal"}" type="number" value="0" min="0">
          </div>`)}
        </div>`)}
      </div>
    `
